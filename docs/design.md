...
- `src/client` is a **client plugin** (declared under `dsh.client`, exported as `./client`, built to a module-table bundle `lib/client.js`). It registers a `Qwen 本地 (vLLM)` page into the settings modal's `settings.section` slot and renders one form over the `llm-qwen-local` section: `baseURL`, an **API Key** field, the model list (id / name / capacities / image budgets / multimodal / `preserveThinking` / reasoning efforts), a **Discover models** button (probes the draft endpoint via `llm.discoverModels` and merges the ids), and **Save** (writes the whole section via `settings.replace`). **There is no route-level `maxRequestImageBytes`** — the field was removed from the plugin entirely (config, schema, and page): every image is inlined once it fits its per-image budget, and an oversized request is refused by the backend LLM service against its own input limits. The host validates the draft against the schema and answers the redacted value back; a schema violation is surfaced inline. Copy is bilingual (zh/en) through the DSH locale registry, and the page refetches on `settings/document-updated` so two open surfaces converge.
